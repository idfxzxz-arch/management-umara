    ...
    <div className="space-y-6">
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }}>
            <Card className="group p-5 transition hover:-translate-y-0.5 hover:shadow-[0_20px_55px_rgba(15,23,42,0.10)]">
              <div className="flex items-center justify-between">
                ...
                <div className={`flex h-10 w-10 items-center justify-center rounded-xl ring-1 transition group-hover:text-white ${statColors[index % statColors.length]}`}><stat.icon className="h-5 w-5" /></div>
              </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <ChartCard title="Pekerjaan Selesai" dotColor="bg-blue-500">
          <LineChart data={data.analytics}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Line type="monotone" dataKey="done" stroke="#3b82f6" strokeWidth={3} /></LineChart>
        </ChartCard>
        <ChartCard title="Kategori Layanan Pajak" dotColor="bg-purple-500">
          <BarChart data={taxCategoryChart}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="category" tick={{ fontSize: 12 }} /><YAxis /><Tooltip /><Bar dataKey="layanan" fill="#8b5cf6" radius={[4, 4, 0, 0]} /></BarChart>
        </ChartCard>
...
function ChartCard({ title, children, dotColor = "bg-blue-500" }: { title: string; children: React.ReactElement; dotColor?: string }) {
  return (
    <Card className="p-5">
