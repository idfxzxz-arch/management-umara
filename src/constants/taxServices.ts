...
export type TaxServiceDefinition = {
...
export const taxServiceDefinitions: TaxServiceDefinition[] = [
  {
...
export function getTaxServicePoint(category: TaxCategory, serviceName: string) {
  return taxServices.find((service) => service.category === category && service.name === serviceName)?.basePoints ?? 0
}
