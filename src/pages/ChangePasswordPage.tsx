...
const schema = z.object({
  ...
  confirm: z.string(),
}).refine((value) => value.password === value.confirm, { path: ['confirm'], message: 'Konfirmasi password tidak sama' })
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-5 dark:bg-slate-950">
      <Card className="w-full max-w-md p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">First login tidak boleh masuk dashboard sebelum password diganti.</p>
        <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <label className="block space-y-2">
            <span className="text-sm font-semibold">Password Baru</span>
            <Input type="password" {...register('password')} />
            {errors.password && <span className="text-xs text-red-600">Minimal 8 karakter, huruf besar, huruf kecil, angka, dan simbol.</span>}
          </label>
          <label className="block space-y-2">
